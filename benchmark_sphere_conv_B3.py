...
def B3_unit_kernel(x, support):

    x_ = x*2/support
    B3 = 2 * 1/12 * ( np.abs(x_-2)**3 - 4 * np.abs(x_-1)**3  + 6 * np.abs(x_)**3 - 4*np.abs(x_+1)**3 + np.abs(x_+2)**3 )
    select = np.abs(x)>support
    B3[select] = 0
    norm = 0.3665191429187533 * support**2
    pixarea = hp.nside2pixarea(nside)
    return B3/norm*pixarea
...
kernel_func = lambda r: B3_unit_kernel(r, support=sigma_rad*3)
...
np.save('kernel_B3.npy', kernel)
np.save('m_B3.npy', part_to_full(m))
np.save('m_B3_smooth_conv.npy', part_to_full(m_smooth_conv))
np.save('m_B3_smooth_healpy.npy', part_to_full(m_smooth_healpy))
